...
export default function CartSidebar() {
  const { cart, removeFromCart, toggleCart, increaseQuantity, decreaseQuantity, isCartOpen, cartTotal } = useCart();
  ...
  return (
    <Sheet open={isCartOpen} onOpenChange={toggleCart}>
      <SheetContent className="bg-[#0a0a0a] border-l border-white/10 text-white w-full sm:w-[400px] flex flex-col p-0 h-full z-100">
                ...
                <div key={item.cartId} className="flex gap-4 group">
                  ...
                  <div className="flex-1 space-y-1">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <button className="text-lg font-bold text-yellow-500 cursor-pointer disabled:text-gray-700" disabled={item.quantity <= 1} onClick={() => decreaseQuantity(item.cartId)}>
                        -
                      </button>
                      <span className="text-xs text-gray-500">Qty: {item.quantity}</span>
                      <button className="text-lg font-bold text-yellow-500 cursor-pointer" onClick={() => increaseQuantity(item.cartId)}>
                        +
                      </button>
                    </div>
                  </div>
